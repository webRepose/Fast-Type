import Style from "../../../styles/Main/Captcha/Captcha.module.scss";
...
const generateCaptcha = (length) => {
  let captcha = "";
  for (let i = 0; i < length; i++) captcha += generateCaptchaChar();
  return captcha;
};
...
const TypingCapcha = () => {
  ...
  if (parse.lang === "English") {
    ...
    captchaChars =
      "АБВГЕЁЖЗИЙКЛМНОПРСТУФХЦЧШЩЫЭЮЯабвгеёжзийклмнопрстуфхцчшщыэюя0123456789";
  }
  ...
  const capchaChange = () => {
    if (document.getElementById(captchaText[iteration].toUpperCase())) {
      if (
        document.getElementById(captchaText[iteration].toUpperCase()) !==
        undefined
      ) {
        document
          .getElementById(captchaText[iteration].toUpperCase())
  ...
  if (document.getElementsByClassName(Style.onKeyClick).length >= 1) {
    ...
    if (captchaText[iteration - 2] !== undefined) {
      document
        .getElementById(captchaText[iteration - 1].toUpperCase())
        .classList.remove(Style.onKeyClick);
      document
        .getElementById(captchaText[iteration - 2].toUpperCase())
        .classList.remove(Style.onKeyClick);
    }
  }
  ...
  const inputCheckOut = (event) => {
    if (!/Android|HarmonyOS/i.test(navigator.userAgent)) {
      if (captchaText[iteration] === event.nativeEvent.data) {
        ...
        setTimeout(() => {
          inputRef.current.style.border = "1px solid #707070";
        }, 500);
          ...
          {window.innerWidth > 967 ? (
            <>
              ...
              <div className={Style.captcha}>
                <img
                  src={`../img/main/captcha/captcha${ran}.png`}
                  alt="background"
                />
                {captchaText && (
                  <span translate="no">
