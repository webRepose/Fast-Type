...
import Style from "../../styles/Components/TypingCode/TypingCode.module.scss";
...
const Typing = ({
      ...
      if (document.getElementsByClassName(Style.onKeyClick).length >= 1) {
        if (klava) {
          if (document.getElementById(inputText[i].toUpperCase())) {
            document
              .getElementById(inputText[i].toUpperCase())
              .classList.remove(Style.onKeyClick);
          }
          if (inputText[i - 2] !== undefined) {
            document
              .getElementById(inputText[i - 1].toUpperCase())
              .classList.remove(Style.onKeyClick);
            document
              .getElementById(inputText[i - 2].toUpperCase())
              .classList.remove(Style.onKeyClick);
          }
        }
      }
    ...
    if (inputText[i] !== undefined) {
      if (klava && document.getElementById(inputText[i].toUpperCase())) {
        if (
          ...
          document.getElementById(inputText[i].toUpperCase())
        ) {
          document
            .getElementById(inputText[i].toUpperCase())
  ...
  const inputCheck = (event) => {
    if (parseMode === "toTime") {
      ...
      if (event.target.value.length === 1)
        setIsTypeWords((prev) => (prev = true));

      if (
        inputText[i] === event.nativeEvent.data &&
    ...
    if (!/Android|HarmonyOS/i.test(navigator.userAgent)) {
      if (inputText[i] === event.nativeEvent.data) {
        ...
        if (document.getElementById("Backspace") !== null || undefined) {
          document
            ...
            .classList.add(Style.onKeyClickBackspace);
          setTimeout(() => {
            document
              .getElementById("Backspace")
              .classList.remove(Style.onKeyClickBackspace);
          }, 300);
        }
      }
      ...
      if (reset + inputText[i] === event.nativeEvent.data) {
        ...
        if (inputArea.current.value.length >= 1)
          setReset((prev) => (prev = inputArea.current.value));
  ...
  const shareMob = () => {
    navigator.share({
      text: t("TI-myRes"),
      url: `https://fasttyping.vercel.app/result?words=${words}&&errors=${errorCount}&&symbols=${simbols}&&time=${parseTime}&&timeSelf=${
        minTimerWords + ":" + secTimerWords
      }&&precent=${
        simbols
          ? Math.round(simbols * (100 / simbols) - errorCount * (100 / simbols))
          : 0
      }`,
      title: t("TI-myRes"),
    });
  ...
  const sharePc = () => setShare((prev) => !prev);

  const Restart = useCallback(() => {
    setKlava((prev) => (prev = false));
    setTimeout(() => setKlava((prev) => (prev = true)), 1);
  ...
  useEffect(() => {
    const handler = (event) => {
      if (share)
        if (!blurCloseRef.current.contains(event.target))
          setShare((prev) => (prev = false));
    };

    document.addEventListener("mousedown", handler);

    const handleClickOutside = (event) => {
      if (inputArea.current && !inputArea.current.contains(event.target)) {
        inputArea.current.focus();
      }
    };
  ...
  const shareRes = `https://fasttyping.vercel.app/result?words=${words}&&errors=${errorCount}&&symbols=${simbols}&&time=${parseTime}&&timeSelf=${
    minTimerWords + ":" + secTimerWords
  }&&precent=${
    simbols
      ? Math.round(simbols * (100 / simbols) - errorCount * (100 / simbols))
            ...
            <div className={Style.partFirst}>
              {allResVsisible && (
                <div className={Style.countAll}>
                  <div>
                    <img src="../img/text-type/chat.svg" alt="words" />
                    {parseMode === "toTime"
                      ? words + " " + t("T-counWord")
                      : `${
                          minTimerWords < 10
                  ...
                  <div>
                    {" "}
                    <img src="../img/text-type/type.svg" alt="words" />
                    {simbols} <p>{t("TR-sym")}</p>
                  </div>
                  <div>
                    <img src="../img/text-type/problem_.svg" alt="words" />{" "}
                    {errorCount} <p>{t("TR-err")}</p>
                  </div>
                </div>
              )}
              {timerVisible && (
                <div className={Style.timer}>
                  {parseMode === "toTime" ? (
                    <samp>
                      <img src="../img/text-type/timer3.0.svg" alt="words" />{" "}
                      {min + ":" + sec}
                    </samp>
                  ) : (
                    <samp>
                      <img src="../img/text-type/timer3.0.svg" alt="words" />{" "}
                      {ourCountWords + " " + t("T-counWord")}
                    </samp>
                  )}
                </div>
              )}
              <div ref={inputBlock} className={Style.inputMain}>
                <div className={Style.inputBlock}>
                  <p translate="no" className={Style.inputText}>
                    {inputText}
                  </p>
                  <textarea
                    translate="no"
              ...
              <GreenButton
                ...
                onClick={() => {
                  setChangeTextNew((prev) => !prev);
                }}
                className={Style.changeText}
              >
                {t("TI-changeText")}
              </GreenButton>
            </div>
            <div className={Style.partTwo}>
              <div className={Style.closeAll}>
                <button
                  ...
                  title={klava ? t("TI-hideKlava") : t("TI-showKlava")}
                  className={Style.closeBlock}
                >
                  {klava ? (
                    <img
                      src="../img/text-type/keyboard_off.svg"
                      alt={t("TI-hideKlava")}
                    />
                  ) : (
                    <img
                      src="../img/text-type/keyboard.svg"
                      alt={t("TI-showKlava")}
                    />
                  )}
                </button>
                <button
                  ...
                  title={timerVisible ? t("TI-hideTimer") : t("TI-showTimer")}
                  className={Style.closeBlock}
                >
                  {timerVisible ? (
                    <img
                      src="../img/text-type/timer_off.svg"
                      alt={t("TI-hideTimer")}
                    />
                  ) : (
                    <img
                      src="../img/text-type/timer.svg"
                      alt={t("TI-showTimer")}
                    />
                  )}
                </button>
                <button
                  onClick={() => {
                    setAllResVisible((prev) => !prev);
                  }}
                  title={allResVsisible ? t("TI-hideCateg") : t("TI-showCateg")}
                  className={Style.closeBlock}
                >
                  {allResVsisible ? (
                    <img
                      src="../img/text-type/category_off.svg"
                      alt={t("TI-hideCateg")}
                    />
                  ) : (
                    <img
                      src="../img/text-type/category.svg"
                      alt={t("TI-showCateg")}
                    />
                  )}
                </button>
                    ...
                    <div className={Style.inputBlurModalsButtons}>
                      <button>
                        ...
                        >
                          ...
                          >
                            <path
                              d="M15.654,27H7.75A1.794,1.794,0,0,1,6,25.25V7.75A1.794,1.794,0,0,1,7.75,6h7.9ZM13.9,25.25V7.75H7.75v17.5ZM17.4,15.6V6H25.25A1.794,1.794,0,0,1,27,7.75V15.6Zm1.75-1.75h6.1V7.75h-6.1ZM17.4,27V17.346H27v7.9A1.794,1.794,0,0,1,25.25,27Zm1.75-1.75h6.1V19.1h-6.1ZM13.9,16.5ZM19.154,13.846ZM19.154,19.1Z"
                      ...
                      <button>
                        ...
                        >
                          ...
                          >
                            <path
                              d="M23.85,42A17.1,17.1,0,0,1,11.2,36.725,17.527,17.527,0,0,1,6,23.95H9A14.678,14.678,0,0,0,13.3,34.6,14.179,14.179,0,0,0,23.85,39,14.541,14.541,0,0,0,34.6,34.55,14.79,14.79,0,0,0,39,23.75a13.964,13.964,0,0,0-4.45-10.475A14.893,14.893,0,0,0,23.85,9a13.6,13.6,0,0,0-6.375,1.55,17.386,17.386,0,0,0-5.175,4.1h5.25v3H7.1V7.25h3v5.3a19.028,19.028,0,0,1,6.175-4.8A17.008,17.008,0,0,1,23.85,6,17.863,17.863,0,0,1,30.9,7.4a18.474,18.474,0,0,1,5.775,3.825,17.956,17.956,0,0,1,3.9,5.675A17.238,17.238,0,0,1,42,23.9a17.592,17.592,0,0,1-1.425,7.05,18.1,18.1,0,0,1-3.9,5.75A18.611,18.611,0,0,1,30.9,40.575,17.592,17.592,0,0,1,23.85,42Zm6.4-9.85-7.7-7.6V13.85h3V23.3L32.4,30Z"
                              transform="translate(-6 -6)"
                              fill="#33d74b"
                            />
                          </svg>
                      ...
                      <button onClick={Restart}>
                        ...
                        >
                          <svg
                            ...
                            width="40"
                            height="40"
                            viewBox="0 0 32.05 37"
                          >
                            <path
                              d="M22.55,41.9A15.315,15.315,0,0,1,12.2,36.85,15.386,15.386,0,0,1,8,26.05,15.442,15.442,0,0,1,9.775,18.8a14.891,14.891,0,0,1,4.975-5.55L16.9,15.4a11.475,11.475,0,0,0-4.35,4.525A12.716,12.716,0,0,0,11,26.05a12.469,12.469,0,0,0,3.3,8.65,12.643,12.643,0,0,0,8.25,4.2Zm3,0v-3a12.743,12.743,0,0,0,8.25-4.225,12.486,12.486,0,0,0,3.25-8.625,12.89,12.89,0,0,0-13-13h-1l3,3L23.9,18.2l-6.65-6.65L23.9,4.9l2.15,2.15-3,3h1A15.4,15.4,0,0,1,35.4,14.725,15.466,15.466,0,0,1,40.05,26.05a15.436,15.436,0,0,1-4.175,10.8A15.223,15.223,0,0,1,25.55,41.9Z"
                      ...
                      <button
                        title={t("TI-share")}
                        onClick={
                          /Android|iPad|iPhone|HarmonyOS|Mobile|Table|iOS|iPod|BlackBerry|WindowsPhone/i.test(
                            navigator.userAgent
                          )
                            ? shareMob
                            : sharePc
